...
impl Preallocated {
    pub fn validate(&self, nft_contract: &NFTContract) -> Option<Flaw> {
        //if let Some(divisibility) = &nft_contract.asset.divisibility {
            if nft_contract.asset.divisibility > 0 {
                return Some(Flaw::DivisibilityInvalid);
            }
       // }
        
        if let Some(supply_cap) = &nft_contract.asset.supply_cap {
            if supply_cap.0 > 1 {
            ...
            */
            ...
            if total_allocations < supply_cap.0 {
                ...
                if let Some(free_mint) = &nft_contract.distribution_schemes.free_mint {
...
impl FreeMint {
    pub fn validate(&self, nft_contract: &NFTContract) -> Option<Flaw> {
        if let Some(supply_cap) = &self.supply_cap {
            ...
            if let Some(super_supply_cap) = &nft_contract.asset.supply_cap {
